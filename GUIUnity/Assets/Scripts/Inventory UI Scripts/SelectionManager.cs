...
public class SelectionManager : MonoBehaviour {
    ...
    private void ProcessClick(UIItemSlot clicked) {
        ...
        if (ItemSlot.Compare(cursor.itemSlot, clicked.itemSlot)) {
            ...
            if (clicked.itemSlot.itemAmount == clicked.itemSlot.item.itemStack)
                return;

            // Otherwise, we add up the amounts and put as much as we can in the clicked slot, leaving the rest in
            // the cursor slot.

            int total = cursor.itemSlot.itemAmount + clicked.itemSlot.itemAmount;
            int maxStack = cursor.itemSlot.item.itemStack; // Cache maxStack for convenience
